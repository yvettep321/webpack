...
BREAKING CHANGE: Asset processing hooks in Compilation has been merged into a single Compilation.hooks.processAssets hook.`;
	...
	_processModuleDependencies(module, callback) {
		...
		/**
		 * @type {Array<{factory: ModuleFactory, dependencies: Dependency[], originModule: Module|null, contextInfo: Partial<ModuleFactoryCreateDataContextInfo>}>}
		 */
	...
	seal(callback) {
		...
		for (const [name, { dependencies, includeDependencies, options }] of this
			.entries) {
			const chunkOptions = this._getChunkOptionsFromEntryOptions(options);
...
Or do you want to use the entrypoints '${name}' and '${runtime}' independently on the same page with a shared runtime? In this case give them both the same value for the 'runtime' option. It must be a name not already used by an entrypoint.`);
	...
	addAsyncEntrypoint(options, module, loc, request) {
		...
		if (runtime) {
			let chunk = this.namedChunks.get(runtime);
			if (!chunk) {
				chunk = this.addChunk(runtime, chunkOptions);
				chunk.preventIntegration = true;
			}
	 ...
	 * @param {ChunkOptions=} options optional chunk options
	 * @returns {Chunk} created chunk (invoked during seal event)
	 */
	addChunk(name, options) {
		if (name) {
		...
		const chunk = new Chunk(name, options);
